...
import 'package:moviehut/category/Details/EnglishWebDetail%20copy.dart';
...
class englishweb extends StatelessWidget {
  // This widget is the root of your application.
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'English Webseries Collections',
...
class _MyHomePageState extends State<MyHomePage> {
 ...
 static List<String> details =[
    ...
    final List<EnglishWebDataModel> engweb = List.generate(
        fruitname.length,
            (index)
        => EnglishWebDataModel('${fruitname[index]}', '${url[index]}','${fruitname[index]}','${details[index]},',''));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.builder(
           itemCount: engweb.length,
          itemBuilder: (context,index){
             return Card(
               child: ListTile
               (
                 title: Text(engweb[index].name),
                 leading: SizedBox(
                   width: 50,
                   height: 50,
                   child: Image.network(engweb[index].ImageUrl),
                 ),
                 onTap: (){
                   Navigator.of(context).push(MaterialPageRoute(builder: (context)=>EnglishWebDetail(fruitDataModel: engweb[index],)));
                 },
               ),
             );
          }
